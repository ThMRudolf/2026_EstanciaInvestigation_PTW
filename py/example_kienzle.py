...
#   fz=0.1, ap=1.0, omega=210.0,
#    time=time, z=2,
#    r_tool=20.0, kappa_deg=90.0, km=1300,
#    # ── noise ──────────────────────────────────────
...
#    # ── noise ──────────────────────────────────────
#    gamma_shape=161.763,            # Gamma fit of std_iq_no_load
...
phi = model.phi_time_series()
Mc_clean = model.torque_time_series(add_noise=False)
Mc = Mc_clean + model.noise_time_series(len(model.time))
ax_a = fig.add_subplot(gs[0, :])
ax_a.plot(phi, Mc_clean, ".",color="steelblue", lw=1.5, label="Mc clean")
ax_a.plot(phi, Mc, "*"   ,    color="grey",    lw=1.5  , label="Mc noisy") #alpha=0.4, lw=0.8
#ax_a.plot(df["time"], df["Mc_maf30"], color="crimson",   lw=1.8, label="MAF n=30")
ax_a.set_xlabel("angle (rad)")
ax_a.set_ylabel("Torque (N·m)")
...
ax_c.set_xlabel("Noise amplitude (N·m)")
...
ax_d.set_ylabel("Peak torque (N·m)")
...
Iq_noisy = Iq_clean + model.noise_time_series(len(time))
ax_e = fig.add_subplot(gs[2, 1])
ax_e.plot(phi, Iq_clean, ".", color="steelblue", lw=1.5, label="Iq clean")
ax_e.plot(phi, Iq_noisy, "*",color="grey",      alpha=0.4, lw=0.8, label="Iq noisy")
